...
`feature/*` ブランチの PR（opened / synchronize / reopened）で CI を自動起動。イメージタグは `pr-<番号>-<短いSHA>`。
...
GitHub Webhook: Payload URL = `EL_URL`、Content type = `application/json`、Secret = `WEBHOOK_SECRET`、Events = **Pull requests** のみ。
...
| PR トリガーで Run が作られない | Webhook の HTTP 202/200、`feature/` ブランチ、EventListener のログ |
